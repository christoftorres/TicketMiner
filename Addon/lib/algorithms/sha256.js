...
// Anders Lindman <https://github.com/AndersLindman>
...
SHA256.nibbleToChar = function(nibble) {
    return String.fromCharCode((nibble < 10 ? 48 : 87) + nibble);
};

SHA256.charToNibble = function(chr) {
    if (chr >= 48 && chr <= 57)
    {
        return chr - 48;
    }
...
SHA256.stringToBytes = function(str) {
    var bytes = SHA256.Uint8Array(str.length);
...
SHA256.hexStringToBytes = function(str) {
    var bytes = SHA256.Uint8Array(str.length>>>1);
    for (var i = 0; i != bytes.length; ++i)
    {
        bytes[i] = SHA256.charToNibble(str.charCodeAt(i<<1 | 0)) << 4;
        bytes[i] |= SHA256.charToNibble(str.charCodeAt(i<<1 | 1));
    }
    return bytes;
};

SHA256.bytesToHexString = function(bytes) {
    var str = "";
    for (var i = 0; i != bytes.length; ++i)
    {
        str += SHA256.nibbleToChar(bytes[i] >>> 4);
        str += SHA256.nibbleToChar(bytes[i] & 0xf);
    }
    return str;
};

SHA256.wordsToBytes = function(words) {
    var bytes = [], word, i, j;
...
SHA256.Int32ToBytes = function(num) {
    arr = SHA256.Uint8Array([
         (num & 0xff000000) >> 24,
         (num & 0x00ff0000) >> 16,
         (num & 0x0000ff00) >> 8,
         (num & 0x000000ff)
...
SHA256.bytesToInt32 = function(bytes) {
